...
minima(CA):-
    ...
    %Nota: Ho bisogno di coda in modo tale da non ripetere piu di una volta delle differenze già fatte.
    %       ex:  alfa-beta è la stessa di beta-alfa (dato che consideriamo il valore assoluto), per cui non ha senso ripeterla
    findall(Diff  , (
                      member(X,ListaDB),
                      coda(X,ListaDB,Coda),
                      scorri(X,Coda,Diff)
          ...
          ListaDifferenze),
    %Siccome ListaDifferenze è una lista di liste, la appiattisco
...
differenza([Gradi1,Primi1,Secondi1],[Gradi2,Primi2,Secondi2],[Diff3,Diff2,Diff1]):-
    ...
    %0: è il primo elemento a cui viene fatta la sottrazione per cui non c è un elemento prima che avrebbe potuto chiedere una decina    %Riporto1 : indica se questa differenza(Diff1) ha necessitato di chiedere una decina
