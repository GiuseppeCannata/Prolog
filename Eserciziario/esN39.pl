%es N 39
% Ok, il cut non è dove dovrebbe essere, guarda esempio libro e
...
merg([A|Ra],[B|Rb],[A|M]) :-
    ...
    (
        ...
        Lung1 > Lung2
        ;
        X1 < X2  %se la condizione sopra non si verifica l ordinamento è alfabetico in ordine crescente
    ),
   merg(Ra,[B|Rb],M).
merg([A|Ra],[B|Rb],[B|M]) :-
    A=[T1|_],
