...
%La prima volta meccerà con questa clausola restituendo LO...
scrivi([H|_],H,y).
% ...appena però provocherò il fallimento per back, farò fallire la
% clausola precedente e il match avverrà con questa
scrivi([_,H1|C],HX,y):-
        write('more(y/n)?'),
        read(X),
        scrivi([H1|C],HX,X).
