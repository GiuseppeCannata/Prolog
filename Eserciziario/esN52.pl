...
%
%esamina va bene
% totale setof non funziona come dovrebbe poiche in A riesco ad ottenere
% tutta la lista solo se faccio fallimento manuale
%
esamina(L1,L2):-
...
totale(A):-
   ...
   %il setof non funziona come mi aspetto e non riesco a capire perche
   setof(int(I,F),( member(int(I,_),ListaPiatta), occorrenza1(I,ListaPiatta,0,F) ), List),
   quicksort(List,A),
